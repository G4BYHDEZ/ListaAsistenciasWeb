...
export default function AdminStudents() {
  ...
  useEffect(() => {
    fetch('http://listaalumnos.utportfolio.cloud:5000/api/admin/courses')
      .then((res) => res.json())
      .then((data) => {
        setCourses(data);
        if (data.length > 0) {
  ...
  useEffect(() => {
    if (selectedCourse) {
      fetch(`http://listaalumnos.utportfolio.cloud:5000/api/courses/${selectedCourse}/attendance/${date}`)
        .then((res) => res.json())
